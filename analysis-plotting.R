...
# load full weights for atus
atuswgts <- read.table("ATUS/2003-2015/atuswgts_0315/atuswgts_0315.dat",header = TRUE,sep = ",")     # ATUS Replicate Weights file
names(atuswgts)[names(atuswgts) == 'TUCASEID'] <- 'id'
...
# create blank df
meta.f.pop <- meta.f[0,c('id','MET.dur','cuttemp','code.f2','weight','elderly','black_white','poverty.f','sex.f')]
meta.f2 <- meta.f[,c('id','MET.dur','cuttemp','code.f2','weight','elderly','black_white','poverty.f','sex.f')]
...
act.t <- ddply(.data=meta.f[!is.na(meta.f$MET.dur),], 
                 .(cuttemp,code.f3), 
                 plyr::summarize, 
                 n=length(code.f3))
...
p26.1 <- ggplot(data=meta.f.pop2[!is.na(meta.f.pop2$MET.dur),], aes(x=cuttemp, y=MET.dur, fill=cuttemp)) +
...
ggsave(paste0(today(),"_MET_exp_by_act_bxplt_300_weighted.tiff"), p26.1, device = "tiff", path = "UAHS Outputs",
...
# labler
n.soc1 <- ddply(.data=meta.f[!is.na(meta.f$MET.dur),], 
                  .(cuttemp,elderly), 
                plyr::summarize, 
                  n=length(elderly))
...
# create activity x MET-time by cuttemp plot
p27.1 <- ggplot(data=meta.f.pop[!is.na(meta.f.pop$MET.dur),], aes(x=cuttemp, y=MET.dur, fill=cuttemp)) +
  geom_boxplot(alpha = 0.7,outlier.size = 0.1) +
  #geom_jitter(aes(group=cut_width(TRTIER1P, 1)), colour = line, size = .7) +
  scale_y_log10(name = "Activity Intensity-time (MET-min)", labels = fmt_dcimals(1), limits=c(1,5000), breaks=c(1,5,10,50,100,500,1000,5000), minor_breaks=log10_minor_break()) + 
  #scale_y_continuous(name = "Exposure (deg-min above 27 deg C)", limits=c(0,6000), breaks = c(0,1000,2000,3000,4000,5000,6000)) + 
...
# BLACK VS NON BLACK
# labler
n.soc2 <- ddply(.data=meta.f[!is.na(meta.f$MET.dur),], 
                .(cuttemp,black_white), 
                plyr::summarize, 
...
# create activity x MET-time by cuttemp plot
p27.2 <- ggplot(data=meta.f.pop[!is.na(meta.f.pop$MET.dur),], aes(x=cuttemp, y=MET.dur, fill=cuttemp)) +
  geom_boxplot(alpha = 0.7,outlier.size = 0.1) +
  #geom_jitter(aes(group=cut_width(TRTIER1P, 1)), colour = line, size = .7) +
...
# MALE VS FEMALE
# labler
n.soc3 <- ddply(.data=meta.f[!is.na(meta.f$MET.dur),], 
                .(cuttemp,sex.f), 
                plyr::summarize, 
                n=length(sex.f))
...
# create activity x MET-time by cuttemp plot
p27.3 <- ggplot(data=meta.f.pop[!is.na(meta.f.pop$sex.f),], aes(x=cuttemp, y=MET.dur, fill=cuttemp)) +
  geom_boxplot(alpha = 0.7,outlier.size = 0.1) +
  #geom_jitter(aes(group=cut_width(TRTIER1P, 1)), colour = line, size = .7) +
  scale_y_log10(labels = fmt_dcimals(1), limits=c(1,5000), breaks=c(1,5,10,50,100,500,1000,5000), minor_breaks=log10_minor_break()) + 
  #scale_y_continuous(name = "Exposure (deg-min above 27 deg C)", limits=c(0,6000), breaks = c(0,1000,2000,3000,4000,5000,6000)) + 
...
ggsave("20180529_MET_exp_socio_bxplt_300_weighted.tiff", socio.plot.an, device = "tiff", path = "UAHS Outputs",
       scale = 1, width = 6.5, height = 5, dpi = 300,units = "in")
...
#msa.drops <- ddply(.data=meta.p, .(msa.f),plyr::summarize,n=length(msa.f))  # instead drop if n < 30
#msa.drops <- msa.drops[msa.drops$n > 30,]
#msa.drops$n_eq <- paste("n =",msa.drops$n)
#msa.drops <- msa.drops[!is.na(msa.drops$msa.f),]

# order factor MSA and CZ labels quartiles of exposure
ord.met.keep <- ddply(.data=meta.pop.keep, 
                 .(msa.f), 
                 plyr::summarize, 
                 q75=quantile(MET.exp27,probs = .75,na.rm = T),
                 q50=quantile(MET.exp27,probs = .50,na.rm = T),
...
n.age.t <- ddply(.data=meta.f2, 
                 .(cuttemp,age.f2), 
                 summarize, 
                 n=length(age.f2))

p.age.t <- ddply(.data=meta, 
                 .(cuttemp,age.f2), 
                 summarize, 
                 p=length(age.f2))
...
n.income.t <- ddply(.data=meta.f3, 
                    .(cuttemp,income.f), 
                    summarize, 
                    n=length(income.f))

p.income.t <- ddply(.data=meta, 
                    .(cuttemp,income.f), 
                    summarize, 
                    p=length(income.f))
...
# add labelers for number of observations by cuttemp and race and for percent of observations outdoors
n.race.t <- ddply(.data=meta.f, 
                  .(cuttemp,race.f2), 
                  summarize, 
                  n=length(race.f2))

p.race.t <- ddply(.data=meta, 
                  .(cuttemp,race.f2), 
                  summarize, 
                  p=length(race.f2))
...
day.t <- ddply(.data=meta.f[!is.na(meta.f$MET.dur),], 
               .(cuttemp,day), 
               plyr::summarize, 
               n=length(day))
